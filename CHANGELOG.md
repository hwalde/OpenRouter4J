...
## [2.0.0] - 2026-09-08
### Changed
- **BREAKING: The builder default model changed from `google/gemini-2.5-flash` to `deepseek/deepseek-v4-flash-0731`.** Requests that never call `.model(...)` now silently send a different model - callers relying on the old default must set one explicitly; callers who always set a model are unaffected. Reason for the change: this release retires every model older than four months from the default and all examples, and `google/gemini-2.5-flash` (June 2025, $0.30/$2.50 per M tokens) no longer qualifies.

...
public final class OpenRouterMessagesRequest extends OpenRouterRequest<OpenRouterMessagesResponse> {
    ...
    public String getBody() {
        ...
        if (hasOrder || hasOnly || hasIgnore
            ...
            if (quantizations != null && !quantizations.isEmpty()) {
                ...
                for (String q : quantizations) { quantArr.put(q); }
    ...
    public static final class Builder
        ...
        /**
         * Adds a {@code role: "system"} message carrying a {@code tool_addition}
         * block - loads a previously deferred tool (declared with
         * {@code defer_loading: true} in the request's {@code tools} array) at
         * this point of the conversation without invalidating the prompt cache.
         * <p>
         * Traps: only valid in {@code role: "system"} messages (this method
         * always emits that role); the referenced tool must have been declared
         * with {@code defer_loading: true}; not supported on Claude Sonnet 5 or
         * models older than Claude Opus 4.8.
         *
         * @param ref the tool reference (plain, MCP tool or MCP toolset)
         * @return this builder
         * @see <a href="https://openrouter.ai/docs/guides/features/tool-calling">Tool calling</a>
         */
        public Builder addToolAdditionMessage(OpenRouterToolReference ref) {
            return addToolAdditionMessage(ref, null);
        }

        /**
         * Adds a {@code role: "system"} message carrying a {@code tool_addition}
         * block with an Anthropic-style {@code cache_control} breakpoint.
         *
         * @param ref the tool reference
         * @param marker the cache marker ({@code null} for none); only the
         *        {@code cacheControl()} form is accepted
         * @return this builder
         * @see #addToolAdditionMessage(OpenRouterToolReference)
         */
        public Builder addToolAdditionMessage(OpenRouterToolReference ref, de.entwicklertraining.openrouter4j.OpenRouterCacheMarker marker) {
            JSONObject block = OpenRouterToolLifecycleBlock.toolAddition(ref, marker).toJson();
            return addMessageWithBlocks("system", List.of(block));
        }

        /**
         * Adds a {@code role: "system"} message carrying a {@code tool_removal}
         * block - removes a tool from the conversation from this point onward
         * without invalidating the prompt cache.
         * <p>
         * Traps: only valid in {@code role: "system"} messages (this method
         * always emits that role); not supported on Claude Sonnet 5 or models
         ...
         * @see <a href="https://openrouter.ai/docs/guides/features/tool-calling">Tool calling</a>
         */
        public Builder addToolRemovalMessage(OpenRouterToolReference ref) {
            return addToolRemovalMessage(ref, null);
        }
         ...
         *        {@code cacheControl()} form is accepted
         * @return this builder
         * @see #addToolRemovalMessage(OpenRouterToolReference)
         */
        public Builder addToolRemovalMessage(OpenRouterToolReference ref, de.entwicklertraining.openrouter4j.OpenRouterCacheMarker marker) {
            JSONObject block = OpenRouterToolLifecycleBlock.toolRemoval(ref, marker).toJson();
            return addMessageWithBlocks("system", List.of(block));
        }

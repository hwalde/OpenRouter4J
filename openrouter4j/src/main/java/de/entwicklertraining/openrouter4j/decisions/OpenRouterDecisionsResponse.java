...
/**
 * Response of POST /api/alpha/decisions: the answers for every requested
 * question key, the resolved judging model, the serving provider and the
 * usage. Recorded wire shape (2026-09-19, live):
 * <pre>{@code
 * {
 ...
 *
 * <p>Trap: this endpoint lives in OpenRouter's alpha namespace and may change
 * or disappear without a major version of the API.
 */
public final class OpenRouterDecisionsResponse extends OpenRouterResponse<OpenRouterDecisionsRequest> {

    OpenRouterDecisionsResponse(JSONObject json, OpenRouterDecisionsRequest request) {
        super(json, request);
    }

    /**
     * @return the decision id, or {@code null} when absent
     */
    public String id() {
        return stringOf("id");
    }
    ...
    /**
     * Typed view of one answer - use {@link OpenRouterDecisionAnswer#type()}
     ...
     */
    public OpenRouterDecisionAnswer answer(String key) {
        JSONObject entry = answers().get(key);
        return entry == null ? null : new OpenRouterDecisionAnswer(key, entry);
    }
     ...
     * your own threshold.
     */
    public static final class OpenRouterDecisionAnswer {

        private final String key;
        private final JSONObject json;

        private OpenRouterDecisionAnswer(String key, JSONObject json) {
            this.key = key;
            this.json = json;
        }

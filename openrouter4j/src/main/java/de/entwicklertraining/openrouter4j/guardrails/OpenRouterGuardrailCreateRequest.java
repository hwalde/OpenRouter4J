...
/**
 * A request to create a new guardrail:
 ...
 *
 * <p>A created guardrail enforces nothing until it is assigned to API keys or
 * members (see the assignment requests); to restrict all traffic in a workspace,
 * update the workspace's default guardrail instead. Every field is emitted only
 * when explicitly configured.
 */
public final class OpenRouterGuardrailCreateRequest extends OpenRouterRequest<OpenRouterGuardrailCreateResponse> {
    ...
    private OpenRouterGuardrailCreateRequest(Builder builder) {
        super(builder);
    ...
    @Override
    public OpenRouterGuardrailCreateResponse createResponse(String responseBody) {
        return new OpenRouterGuardrailCreateResponse(new JSONObject(responseBody), this);
    }

    /**
     * Starting point for building a {@link OpenRouterGuardrailCreateRequest}.
     */
    public static final class Builder extends ApiRequestBuilderBase<Builder, OpenRouterGuardrailCreateRequest> {
 ...
 */
    ...
    /**
 * Sets the body field {@code workspace_id} (optional) - the workspace to
 * create the guardrail in. When omitted, the guardrail is created in the default
 * workspace; if that default has been deleted, the request returns a 400 and the
 * {@code workspace_id} must be passed explicitly.
 */
    public Builder workspaceId(String workspaceId) {
        ...
        @Override
        public OpenRouterGuardrailCreateRequest build() {
        ...
        if ((limitUsd == null) != (resetInterval == null)) {
            ...
            return new OpenRouterGuardrailCreateRequest(this);
        }


    @Override
    public OpenRouterGuardrailCreateResponse execute() {
        return client.sendRequest(build());
    }

    @Override
    public OpenRouterGuardrailCreateResponse executeWithRetry() {
        return client.sendRequestWithRetry(build());
    }

    @Override
    public OpenRouterGuardrailCreateResponse executeWithExponentialBackoff() {

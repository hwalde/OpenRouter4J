...
* Server-side plugins - typed implementations for all eleven plugin ids (`web` search, `auto-router`, `auto-beta-router`, `moderation`, `web-fetch`, `file-parser`, `response-healing`, `context-compression`, `pareto-router`, `fusion`, `switchyard-router`) plus a generic escape hatch; built-in server tools typed as well (`openrouter:web_search` with `userLocation(...)` for location-aware results and `xSearch(...)` for X/Twitter search alongside native web search - native-search providers only, billed separately, `openrouter:web_fetch`, `openrouter:datetime`, `openrouter:advisor`, `openrouter:bash`, `openrouter:shell`, `openrouter:apply_patch`, `openrouter:files`, `openrouter:fusion`, `openrouter:image_generation`, `openrouter:experimental__search_models`, `openrouter:subagent`, `openrouter:tool_search`) and multimodal output (`modalities` / `image_config`, with `images`/`audio` response accessors; note that the audio output path currently has no runnable example because only OpenAI models serve it)

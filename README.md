...
* Deferred tool loading: `deferLoading(true)` on a tool definition keeps large tool catalogs out of the prompt until tool search reveals them - on Chat Completions via the provider-managed path (Anthropic models only, no `openrouter:tool_search` server tool needed; that server tool is only served by the Responses and Anthropic Messages APIs)

...
* Server-side plugins - typed implementations for all eleven plugin ids (`web` search, `auto-router`, `auto-beta-router`, `moderation`, `web-fetch`, `file-parser`, `response-healing`, `context-compression`, `pareto-router`, `fusion`, `switchyard-router`) plus a generic escape hatch; built-in server tools typed as well (`openrouter:web_search` with `userLocation(...)` for location-aware results and `xSearch(...)` for X/Twitter search alongside native web search - native-search providers only, billed separately, `openrouter:web_fetch`, `openrouter:datetime`, `openrouter:advisor`, `openrouter:bash`, `openrouter:shell`, `openrouter:apply_patch`, `openrouter:files`, `openrouter:fusion`, `openrouter:image_generation`, `openrouter:experimental__search_models`, `openrouter:subagent`, `openrouter:tool_search`), the OpenAI Responses-syntax web search shorthand (`web_search_preview` via `OpenRouterWebSearchShorthandTool` / `OpenRouterServerTool.webSearchShorthand()` - emitted verbatim, the API converts it to `openrouter:web_search`; prefer the typed `openrouter:web_search` form for new code) and multimodal output (`modalities` / `image_config`, with `images`/`audio` response accessors; note that the audio output path currently has no runnable example because only OpenAI models serve it)
...
* **Response caching** (OpenRouter-level, provider-independent): `responseCache(Boolean)` / `responseCacheClear(Boolean)` / `responseCacheTtl(Integer)` on the chat, Responses, Messages and Embeddings builders emit the `X-OpenRouter-Cache` header family - identical requests are answered from cache with zeroed usage and no billing. Traps: the cache key includes the JSON property order (not whitespace) and omitted fields differ from explicit defaults; cached responses are returned verbatim regardless of `temperature` (use `responseCacheClear(true)` or a short TTL for freshness); a preset with `cache_enabled: false` cannot be overridden. Plus a generic `header(name, value)` escape hatch for headers the library does not type (CR/LF rejected loudly; on chat it rides the tool-call loop). Example: `OpenRouterResponseCachingExample`
...
* **Embeddings**: embedding vectors via `POST /embeddings` (`client.embeddings()` - required `model` plus `input(String)` or batch `inputs(List)`; optional `dimensions`, `encodingFormat("float"|"base64")`, `inputType`, `user`, `sessionId` and `trace(OpenRouterTraceConfig)` and full provider preferences (`dataCollection`, `quantizations`, `sort`/`sortBy`, `maxPrice`, `preferredMaxLatency`/`preferredMinThroughput`, `enforceDistillableText`, `zdr`) alongside the routing subset `providerOrder` / `providerOnly` / `providerIgnore` / `requireParameters` / `allowFallbacks`), typed response with `embeddings()` / `embedding(index)` and token usage; base64 vectors decode via `vectorOrDecoded()`. Trap: `dimensions` is only accepted by models with configurable dimensions, others reject it with HTTP 400. Embedding-model listing via `client.embeddingsModels()` (`GET /embeddings/models`). Example: `OpenRouterEmbeddingsExample`
* **Rerank**: relevance-sorted document reranking via `POST /rerank` (`client.rerank()` - required `model`, `query` and at least one document via `addDocument(String)` (bare JSON string) or the structured object form `addDocument(String text, String image)`; optional `topN`, `user(...)`, `sessionId(...)` and `trace(OpenRouterTraceConfig)` and the same full provider preferences as the embeddings endpoint), typed results with `index()` (position in the input list), `relevanceScore()` and the echoed document, plus `searchUnits()` (Cohere billing unit). Trap: a structured document needs text or image, and the image must be a URL or a `data:image/...` URI. Example: `OpenRouterRerankExample`
* **Batch API**: `client.batches()` - asynchronous batches of inference requests on `:batch` endpoint variants (typically ~50% of the standard per-token pricing, `24h` completion window): `submit()` (`POST /batches` - required `endpoint` (chat completions / Responses / Anthropic Messages / embeddings), batch-level `model` and the `requests` array of `OpenRouterBatchItem`s with `custom_id` + a body in the chosen endpoint's shape (raw via `OpenRouterBatchItem.of`, or reused verbatim from an existing inference request via `OpenRouterBatchItem.fromRequest`), optional `providerOnly(...)` pinning (the only provider preference the Batch API accepts) and `completionWindow("24h")`; the body serializes `endpoint`/`model`/`provider`/`completion_window` before `requests` because the API stream-parses and answers 400 otherwise), `list()` (`GET /batches` - `limit` 1-100, `after` cursor, the repeatable `status` filter and `createdAfter`/`createdBefore`, plus `queryParam`), `get(id)` (polls status and returns the inline `results` once completed) and `delete(id)` (terminal batches only - in-flight answers 409; purges artifacts instead of waiting out the 30-day retention). Typed `OpenRouterBatch` view (`isTerminal()`, `requestCounts()`, `usage()`, `results()`/`result(customId)`) and per-result typed bodies (`chatCompletionResponse()` / `messagesResponse()` / `responsesResponse()` / `embeddingsResponse()`). Traps: a submit answers 202 `validating` - submission success is not request success (per-request problems move the whole batch to `failed` after the 202); the model needs a `:batch` variant (400 without one, 404 when no listed provider has one); multimodal input must be URL-only, audio/video parts, `stream: true`, `speed` and OpenRouter-orchestrated web search are rejected, and on Google models every request must ask for the same `response_format`. Example: `OpenRouterBatchExample`
...
* **Anthropic Messages API**: `client.messages()` issues `POST /messages` - the `system` prompt travels as a top-level field, `maxTokens` is required by Anthropic semantics (rejected loudly when missing), the effort setting travels in `output_config` (`effort`, `outputFormat`, `taskBudget`), and the response carries typed views over the content blocks (`textBlocks()`, `toolUseBlocks()`, `thinkingBlocks()`, ...). Streaming via `stream(handler)`: every Anthropic event (`message_start`, `content_block_delta`, `message_stop`, ...) arrives as its raw JSON string. The server-tool agent loop can be bounded with `stopServerToolsWhen(...)` (same `OpenRouterStopCondition` factories as chat: OR logic, overrides `max_tool_calls`, ends with one final turn with tools disabled), and long agentic conversations shed old tool results, thinking blocks or compact server-side via `contextManagement(...)` (the three Anthropic strategy types `clear_tool_uses_20250919`, `clear_thinking_20251015`, `compact_20260112` plus a verbatim escape hatch). Plus Anthropic server-side safeguards via `safeguards(...)` / `addSafeguard(...)` (`OpenRouterSafeguard.of(type[, classifierContext])`, typed `safeguardResults()` on the response) thinking presentation/binding via `thinkingDisplay(...)` / `thinkingBlockBinding(...)` (require an enabled thinking mode; the deprecated `mismatch_behavior` alias via `thinkingBlockBindingRaw(...)`), mid-conversation tool lifecycle via `addToolAdditionMessage(OpenRouterToolReference)` / `addToolRemovalMessage(...)` (system-role `tool_addition`/`tool_removal` blocks; `tool_addition` requires `deferLoading(true)` on the referenced tool; not supported on Claude Sonnet 5 or pre-Opus-4.8 models), and full provider preferences (`dataCollection`, `quantizations`, `sort`/`sortBy`, `maxPrice`, `preferredMaxLatency`/`preferredMinThroughput`, `enforceDistillableText`, `zdr`) alongside the routing subset. Example: `OpenRouterMessagesExample`
* **OpenAI Responses API**: `client.responses()` issues `POST /responses` - the successor surface for features that only exist there (full `openrouter:tool_search` support, `openrouter:apply_patch`, subagent function inheritance). Input as a plain string or item array (`addMessage(role, text)`, typed `OpenRouterInputItem` items via `addInput(...)` - `functionCall`/`functionCallOutput`/`outputMessage`/`reasoning`/`itemReference` for replaying a tool-call turn, since the API is stateless - plus a verbatim `addInputItem` escape hatch), typed sampling/reasoning/tool options reusing the chat plugin and server-tool types, full provider preferences (`dataCollection`, `quantizations`, `sort`/`sortBy`, `maxPrice`, `preferredMaxLatency`/`preferredMinThroughput`, `enforceDistillableText`, `zdr`) alongside the routing subset, `tool_choice` object forms via `toolChoiceFunction(name)` (flat `{"type":"function","name":...}`) / `toolChoiceAllowedTools(mode, refs)` / `toolChoiceType(type)` (precedence: named function > allowed_tools > tool-type > plain keyword), stored prompt templates via `prompt(id)` + `promptVariable(key, value)`, image generation via `imageConfig(OpenRouterImageConfig)`, `debug(echoUpstreamBody)`, the typed `text` output options (`textFormat` / `textVerbosity`), typed citations on the message output's `output_text` parts (`OpenRouterTextAnnotation` via `outputTextParts()` / `annotations()` - `urlCitation`/`fileCitation`/`filePath`, with `start_index`/`end_index` as character offsets into the part's `text`), `topLogprobs(n)` and `promptCacheOptions(mode[, ttl])` for the explicit-cache setup, typed OpenAI-native tools declared on the Responses `tools` array (`OpenRouterCustomTool` for client-executed freeform tools with `text`/`grammar` format and the `async` flag - the result comes back as `function_call_output` with the original `call_id`; `OpenRouterMcpServerTool` for third-party MCP tool servers with both `allowed_tools` wire forms; `OpenRouterCodeInterpreterServerTool` with both `container` wire forms and a validated `memory_limit` (the two-argument `containerAuto` emits an explicit JSON `null` for "no explicit limit", the shorter forms omit the field); `OpenRouterComputerUseServerTool`; `OpenRouterFileSearchServerTool` with comparison and compound filters plus `ranking_options` - flat top-level fields, no `parameters` wrapper; the chat-completions `tools` union lists none of these five - the four `OpenRouterServerTool` implementations (`mcp`, `code_interpreter`, `computer_use_preview`, `file_search`) can still be sent through `serverTools(...)` only as an escape-hatch use at the caller, while `OpenRouterCustomTool` deliberately does not implement `OpenRouterServerTool` and cannot reach the chat builder at all), and `stream(handler)` delivering every Responses event as raw JSON. The API is stateless - `previous_response_id` is rejected, send the full history in `input`. Example: `OpenRouterResponsesExample`
* **System One API**: `client.systemOne()` issues `POST /systemone` - evaluate content against typed questions with a System One model (TypeSafe Jev family): boolean `noul`, multi-class `choice` and ordered `score` (the same typed `OpenRouterDecisionQuestion` factories as Decisions). Related but distinct from Decisions: the plain `/api/v1` path (no alpha namespace) and bare model ids (`jev-1.13`, `jev-latest` mapped onto `typesafe/`). Trap: ordinary chat model ids are rejected with 400. Example: `OpenRouterSystemOneExample`
...
* **Workspace and organization management**: the `client.workspaces()` surface - `list()` / `create()` (name plus URL-friendly `slug`, validated loudly) / `get(id)` / `update(id)` (`slug` renames the workspace) / `delete(id)`, the members endpoints `members(id)` / `addMembers(id)` / `removeMembers(id)`, the budget endpoints `budgets(ref)` / `budget(ref, interval)` / `upsertBudget(ref, interval)` (limit in USD per `daily` / `weekly` / `monthly` / `lifetime`; `includeByokInBudgets` is a workspace-wide setting) / `deleteBudget(ref, interval)`, and `disabledServerTools(...)` / `addDisabledServerTool(...)` governance (`disabled_server_tools` - a request naming a disabled tool is rejected with HTTP 403; an empty array clears; unset omits the field (create: API default inherit/none, update: the stored list stays unchanged)); plus `client.organization().members()`. Example: `OpenRouterWorkspacesExample`
